...
-- whether or not VF uses 'getcallingscript()' to get the script (default is false because detection)
local useGetCallingScript = false

--- used to enable/disable VF's keyToString for remotes
...
--- @param value any
--- @return string
function VF:ValueToVar(value, variablename)
	assert(variablename == nil or typeof(variablename) == "string", "string expected, got " .. typeof(variablename))
	if not variablename then
		variablename = 1
	end
	return VF.v2v({ [variablename] = value })
end

--- Converts any value to a string, cannot preserve function contents
--- @param value any
--- @return string
function VF:ValueToString(value)
	return VF.v2s(value)
end

--- Generates the VF function info
--- @param func function
--- @return string
function VF:GetFunctionInfo(func)
...
function VF.v2s(v, l, p, n, vtv, i, pt, path, tables, tI)
	if not tI then
		tI = { 0 }
	else
		tI[1] += 1
	...
	elseif typeof(v) == "string" then
		return VF.formatstr(v, l)
	elseif typeof(v) == "function" then
		return VF.f2s(v)
	elseif typeof(v) == "table" then
		return VF.t2s(v, l, p, n, vtv, i, pt, path, tables, tI)
	...
	elseif type(v) == "vector" then
		return string.format("Vector3.new(%s, %s, %s)", VF.v2s(v.X), VF.v2s(v.Y), VF.v2s(v.Z))
	else
		return "nil --[[" .. typeof(v) .. "]]"
	end
end
...
function VF.v2v(t)
	...
	for i, v in pairs(t) do
		if type(i) == "string" and i:match("^[%a_]+[%w_]*$") then
			ret = ret .. "local " .. i .. " = " .. VF.v2s(v, nil, nil, i, true) .. "\n"
		elseif tostring(i):match("^[%a_]+[%w_]*$") then
			ret = ret
				...
				.. tostring(count)
				.. " = "
				.. VF.v2s(v, nil, nil, tostring(i):lower() .. "_" .. tostring(count), true)
				.. "\n"
		else
			ret = ret
				.. "local "
				...
				.. tostring(count)
				.. " = "
				.. VF.v2s(v, nil, nil, type(v) .. "_" .. tostring(count), true)
				.. "\n"
		end
		count = count + 1
	end
...
--- @param tables table
--- @param tI table
function VF.t2s(t, l, p, n, vtv, i, pt, path, tables, tI)
	...
	for k, v in pairs(t) do -- iterates over table
		...
		if rawequal(k, t) then -- checks if the table being iterated over is being used as an index within itself (yay, lua)
			bottomstr = bottomstr
				...
				.. (
					...
					or VF.v2s(v, l, p, n, vtv, k, t, path .. "[" .. tostring(n) .. tostring(path) .. "]", tables)
				)
			size -= 1
			continue
		end
		...
		if type(k) == "string" and k:match("^[%a_]+[%w_]*$") then -- cleanly handles table path generation (for the first half)
			...
			currentPath = "[" .. VF.k2s(k, l, p, n, vtv, k, t, path .. currentPath, tables, tI) .. "]"
		end
		if size % 100 == 0 then
			scheduleWait()
		end
		...
		s = s
			...
			.. string.rep(" ", l)
			.. "["
			.. VF.k2s(k, l, p, n, vtv, k, t, path .. currentPath, tables, tI)
			.. "] = "
			.. VF.v2s(v, l, p, n, vtv, k, t, path .. currentPath, tables, tI)
			.. ","
	end
	...
	if keyToString then
		if typeof(v) == "userdata" and getrawmetatable(v) then
			return string.format(
				'"<void> (%s)" --[[Potentially hidden data (tostring in VF:HookRemote/GetRemoteFiredSignal at your own risk)]]',
				VF.safetostring(v)
			)
		elseif typeof(v) == "userdata" then
			return string.format('"<void> (%s)"', VF.safetostring(v))
...
function VF.f2s(f)
	for k, x in pairs(getgenv()) do
		...
		elseif type(x) == "table" then
			isgucci, gpath = VF.v2p(f, x)
		end
		if isgucci and type(k) ~= "function" then
			if type(k) == "string" and k:match("^[%a_]+[%w_]*$") then
				return k .. gpath
			else
				return "getgenv()[" .. VF.v2s(k) .. "]" .. gpath
			end
		end
	end
		...
		while true do
			if parent and parent == player.Character then
				if player == Players.LocalPlayer then
					...
					return VF.i2p(player) .. ".Character" .. out
				end
			else
				if parent.Name:match("[%a_]+[%w+]*") ~= parent.Name then
					out = ":FindFirstChild(" .. VF.formatstr(parent.Name) .. ")" .. out
				else
					out = "." .. parent.Name .. out
				end
			end
			...
			if parent and parent.Parent == game then
				...
				if service then
					...
					if parent.Name:match("[%a_]+[%w_]*") then
						...
						return "game:FindFirstChild(" .. VF.formatstr(parent.Name) .. ")" .. out
					end
				end
			elseif parent.Parent == nil then
				getnilrequired = true
				return "getNil(" .. VF.formatstr(parent.Name) .. ', "' .. parent.ClassName .. '")' .. out
...
function VF.u2s(u)
	...
	elseif typeof(u) == "Ray" then
		-- Ray
		return "Ray.new(" .. VF.u2s(u.Origin) .. ", " .. VF.u2s(u.Direction) .. ")"
	...
	elseif typeof(u) == "Region3" then
		...
		return "Region3.new(" .. VF.u2s(vector1) .. ", " .. VF.u2s(vector2) .. ")"
	...
	elseif typeof(u) == "Vector3" then
		return string.format("Vector3.new(%s, %s, %s)", VF.v2s(u.X), VF.v2s(u.Y), VF.v2s(u.Z))
	...
	elseif typeof(u) == "DockWidgetPluginGuiInfo" then
		return string.format(
			...
			VF.v2s(u.InitialEnabled),
			VF.v2s(u.InitialEnabledShouldOverrideRestore),
			VF.v2s(u.FloatingXSize),
			VF.v2s(u.FloatingYSize),
			VF.v2s(u.MinWidth),
			VF.v2s(u.MinHeight)
		)
	elseif typeof(u) == "PathWaypoint" then
		return string.format("PathWaypoint.new(%s, %s)", VF.v2s(u.Position), VF.v2s(u.Action))
	elseif typeof(u) == "UDim" then
		return string.format("UDim.new(%s, %s)", VF.v2s(u.Scale), VF.v2s(u.Offset))
	...
	elseif typeof(u) == "Rect" then
		return string.format("Rect.new(%s, %s)", VF.v2s(u.Min), VF.v2s(u.Max))
	else
	...
	for i, v in pairs(t) do
		...
		if type(v) == "table" then
			...
			if not duplicate then
				table.insert(prev, t)
				local found
				found, p = VF.v2p(x, v, path, prev)
				if found then
					if type(i) == "string" and i:match("^[%a_]+[%w_]*$") then
						return true, "." .. i .. p
					else
						return true, "[" .. VF.v2s(i) .. "]" .. p
					end
				end
			end
		end
...
function VF.formatstr(s, indentation)
	if not indentation then
		indentation = 0
	end
	local handled, reachedMax = VF.handlespecials(s, indentation)
	return '"'
		.. handled
		.. '"'
		.. (
			reachedMax
				and " --[[ MAXIMUM STRING SIZE REACHED, CHANGE '_G.VFMaxStringSize' TO ADJUST MAXIMUM SIZE ]]"
			or ""
		)
end

--- Adds \'s to the text as a replacement to whitespace chars and other things because string.format can't yayeet
function VF.handlespecials(value, indentation)
	local buildStr = {}
	local i = 1
...
-- safe (ish) tostring
function VF.safetostring(v: any)
...
--- @param src string
function VF.getScriptFromSrc(src)
	local realPath
	local runningTest
	--- @type number
	local s, e
